...
/*HACKITY HACK UPCOMING*/
//KALMI: Well obviously this was not here....
#include "../../json/json.h"

ssize_t JSPipeMount::Write(ino_t slot, off_t offset, const void *buf, size_t count) {
  assert(count >= 0);
  ...
  Json::Value root;
  Json::Reader reader;
  bool parsingSuccessful = reader.parse("{ \"command\" : \"debug\", \"data\": { \"str\": \"\" } }", root); 
  if(!parsingSuccessful){
    std::string error("{ \"command\" : \"exception\", \"data\": { \"str\": \"Internal error. (JSPipeMount::Write)\" } }");
    outbound_bridge_->Post(error.c_str(), error.length());
  }else{
    root["data"]["str"] = std::string("stderr/stdout: ") + std::string((const char*)buf, count); 
    Json::StyledWriter writer; 
    std::string json = writer.write(root);
    outbound_bridge_->Post(json.c_str(), json.length());
  }
