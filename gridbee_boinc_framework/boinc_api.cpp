...
void PostMessage(const std::string& message); //as seen in worker.cc


extern Json::Value checkpoint_data;
void kalmi_set_checkpoint_data(const Json::Value data)
{
  checkpoint_data = data;
}
Json::Value kalmi_get_checkpoint_data()
{
...
void _writeAllFiles()
{
	try
	{
		...
		if (dp == NULL)
		{
			PostMessage("Could not open dir");
			return;
		}
		...
		while((entry = readdir(dp)))
		{
			...
			json["command"]=Json::Value("replace");
			
			std::ostringstream namebuffer;
			namebuffer << entry->d_name;
			
			if(namebuffer.str()=="work_unit.sah")
			{
				continue;
			}
			
			data["name"]=Json::Value(namebuffer.str());
			
			std::ostringstream contentbuffer;  
			/*unsigned*/ char* filecontent = read_whole_file(namebuffer.str().c_str());    
			
			contentbuffer << r_base64_encode(filecontent, strlen(filecontent));
			//contentbuffer << filecontent;
			
			free(filecontent);
			data["str"]=Json::Value(contentbuffer.str());
					
			json["data"]=data;		
			
			std::ostringstream buffer;  
			buffer << json;
			PostMessage(buffer.str());
		}
	...
	catch (void* e)
	{
		...
		buff << "_writeAllFiles: " << e;
		PostMessage(buff.str());
	}
}
...
static int setup_shared_mem() {
    ...
    fprintf(stderr,
        "%s BUG(!): Should be in standalone mode!\n",
        boinc_msg_prefix(buf, sizeof(buf))
    );
    return 0;
...
static bool update_app_progress(double cpu_t, double cp_cpu_t) {
    ...
    std::stringstream ss;
    ss << "{\"command\":\"progress\", \"data\": " << fraction_done << " }";
    PostMessage(ss.str());    
...
int boinc_init_options_general(BOINC_OPTIONS& opt) {
    ...
    if (retval)
        ...
        if (retval)
				{
            fprintf(stderr, "%s Can't set up shared mem: %d. Will run in standalone mode.\n",
...
int boinc_finish(int status)
{
    ...
    _writeAllFiles();
    std::stringstream ss;
    ss << "{\"command\":\"done\", \"data\":{\"exitStatus\": " << status << "}}";
    PostMessage(ss.str());
...
void boinc_exit(int status)
{
    ...
    if (options.main_program && file_lock.locked)
        ...
        if (retval)
            ...
            fprintf(stderr,"%s Can't unlock lockfile (%d): %s\n", boinc_msg_prefix(buf, sizeof(buf)), retval, buf);
					#else
            fprintf(stderr, "%s Can't unlock lockfile (%d)\n", boinc_msg_prefix(buf, sizeof(buf)), retval);
...
int boinc_parse_init_data_file()
{
    ...
    aid.clear();
    aid.checkpoint_period = 30;//DEFAULT_CHECKPOINT_PERIOD;

    if (!boinc_file_exists(INIT_DATA_FILE))
		{
        fprintf(stderr, "%s Can't open init data file - running in standalone mode\n",
								boinc_msg_prefix(buf, sizeof(buf)));
        return ERR_FOPEN;
    }
    ...
    if (retval) 
		{
        fprintf(stderr, "%s Can't parse init data file - running in standalone mode\n",
								boinc_msg_prefix(buf, sizeof(buf)));
        return retval;
    }
    return 0;
...
int boinc_report_app_status(double cpu_time, double checkpoint_cpu_time, double _fraction_done)
{
...
static inline void handle_graphics_messages()
{
    ...
    if (app_client_shm->shm->graphics_request.get_msg(buf))
        ...
        app_client_shm->shm->graphics_reply.send_msg(xml_graphics_modes[m.mode]);
    }
}

static void graphics_cleanup()
{
    if (!have_graphics_app)
			return;
...
static void timer_handler()
{
    ...
    if (in_critical_section)
		{
        fprintf(stderr, "%s: timer_handler(): in critical section\n",
								boinc_msg_prefix(buf, sizeof(buf)));
    }
#endif
    ...
    if (interrupt_count % TIMERS_PER_SEC)
			return;
    ...
    if (ga_win.pid)
      ...
      if (! ga_win.is_running())
			{
				app_client_shm->shm->graphics_reply.send_msg(xml_graphics_modes[MODE_HIDE_GRAPHICS]);
      }
    }
...
int boinc_checkpoint_completed()
{
   ...
   _writeAllFiles();  
    ...
    std::ostringstream buffer;  
    buffer << "{\"command\" : \"checkpoint\", \"data\" : " << checkpoint_data << "}";
